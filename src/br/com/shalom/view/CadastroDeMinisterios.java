 ...
 * @author gibran
 */
public class CadastroDeMinisterios extends javax.swing.JInternalFrame {

    /**
     * Creates new form CadastroDePessoas
     */
    public CadastroDeMinisterios() {
    ...
    private void initComponents() {
        ...
        jLabel4.setText("Cadastro de Ministérios");
    ...
    private void btListarPessoasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btListarPessoasActionPerformed
        ...
        List<Ministerio> ministerios = daoMinisterio.getAllMinisterios(session);
        session.clear();
        ministerios.forEach((Ministerio m) -> {
            tableModel.addRow(new String[]{m.getId().toString(), m.getNome()});
        });
    ...
    private void jTableKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jTableKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER && (estado == Estado.ATUALIZACAO || estado == Estado.EXTINCAO)) {
            ...
            m = (Ministerio) s.load(Ministerio.class, Integer.parseInt(jTable.getValueAt(jTable.getSelectedRow(), 0).toString()));
            jTextId.setText(m.getId().toString());
            jTextNome.setText(m.getNome());
        }
    ...
    private void btSalvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btSalvarActionPerformed
        Ministerio m = getMinisterio();
        gerenciarEstados(m);
        btListarPessoasActionPerformed(evt);
        desabilitarCamposEdicao();
    }//GEN-LAST:event_btSalvarActionPerformed

    private void gerenciarEstados(Ministerio m) {
        if (estado == Estado.INSERCAO) {
            daoMinisterio.adicionarMinisterio(JPAUtil.getSession(), m);
        }
        if (estado == Estado.ATUALIZACAO) {
            daoMinisterio.atulizarMinisterio(JPAUtil.getSession(), m);
        }
        if (estado == Estado.EXTINCAO) {
            daoMinisterio.removerMinisterio(JPAUtil.getSession(), m);
        }

        estado = Estado.NEUTRO;
    }

    private Ministerio getMinisterio() {
        Ministerio m = new Ministerio();
        if (estado == Estado.EXTINCAO) {
            m.setId(Integer.parseInt(jTextId.getText().trim()));
            return m;
        }
        if (estado == Estado.ATUALIZACAO) {
            m.setId(Integer.parseInt(jTextId.getText().trim()));
        }
        m.setNome(jTextNome.getText().trim());
        return m;
    }
